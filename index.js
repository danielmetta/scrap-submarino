  ...
  return new Promise(function(acept, error) {
    ...
    request(options, function(error, response, html) {
      if(!error) {
        ...
        product.price = $('.mp-details').children('meta[itemprop=lowPrice]').attr('content');
        product.image = $('.mp-photos').children('.carousel').children('.carousel-list').children().first().children('img[itemprop=thumbnail]').attr('data-szimg');
        if (product.image === undefined) {
            product.image = $('.mp-photos').children('.carousel').children('.carousel-list').children().first().children('img[itemprop=thumbnail]').attr('src');
        }
        acept(product);
      } else {
